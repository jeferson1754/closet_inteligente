...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_usage_delta'])) {
        ...
        if ($stmt = $mysqli_obj->prepare($sql_update)) {
            $stmt->bind_param("ii", $delta, $prenda_id);
            if ($stmt->execute()) {
                $response['success'] = true;
                $response['message'] = ($delta > 0) ? '¡Uso sumado!' : 'Uso restado correctamente.';

                // Registrar en historial solo si es incremento
                if ($delta > 0) {
                ...
                $response['message'] = 'Error al actualizar usos: ' . $stmt->error;
            }
            $stmt->close();
        } else {
