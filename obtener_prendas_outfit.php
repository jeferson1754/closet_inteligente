...
if (isset($_GET['outfit_id']) && is_numeric($_GET['outfit_id'])) {
    ...
    if ($stmt = $mysqli_obj->prepare($sql_prendas_ids)) {
        ...
        if (!empty($prendas_ids)) {
            ...
            $sql_detalles_prendas = "SELECT id, nombre, tipo, color_principal, foto, detalles_adicionales FROM prendas WHERE id IN ($placeholders)  ORDER BY FIELD(LOWER(tipo), $orden_prendas) ASC";
            // Añadido 'comentarios'
